...
#     while changed:
    for run_times in range(1):
        ...
        #nearestpoint=nearestpoint.evaluate()
        #nearestid=nearestid.evaluate()
#             print nearestpoint
#             print nearestid
        #print "OVER2"
        for i in range(K):
...
def benchmark_kmeans(ctx, timer):
    ...
    while count<5: 
        ...
        kmeans(N, K, means, v)
        time_end=datetime.now()
        print "When K is ",K,",Total time is ",time_end-time_begin
        K=K*2
        count=count+1
