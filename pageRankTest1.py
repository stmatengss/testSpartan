...
def calvalue():
    ...
    #print res
    pro=-1
    degree=np.ones((NODES,1),dtype=np.float32)*INF
...
#     print value,source,dest
    ...
    wts=eager(expr.from_numpy(data,tile_hint=(num_pages,PAGES_PER_WORKER/8)))
#     print wts.evaluate()
#     print wts.glom()
...
#         print wts
#         print tmp1.shape
        tmp2=(expr.dot(wts, tmp1))   #4
#         print tmp2
#         print tmp2
#         tmp2.evaluate().glom()
        new_p=DI*tmp2
        new_p=new_p+(1-DI)*ones  #5
        diff=eager(expr.sum(expr.abs(p-new_p))).glom()  #6
        print diff
        p=new_p  #7
        print p.evaluate().glom()
        time_end=datetime.now()
